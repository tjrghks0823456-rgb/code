...
2. **메타인지 격차 (Meta-gap) 대조**:
   ...
   - 6축 점수는 단일 지표가 아니라 여러 하위 지표의 가중 조합으로 계산되며, 데이터가 부족한 하위 지표는 0점 처리하지 않고 제외한 뒤 남은 지표로 재가중 평균을 적용합니다.
   - 검색 기록, 출처 정보, NLP 카테고리, 실제 시청 지속 시간이 부족하면 일부 지표는 참고용으로 표시됩니다.
   - 본 서비스는 심리 진단이 아니라 시청 기록 기반 행동 경향 점수화 및 해석 서비스입니다.
