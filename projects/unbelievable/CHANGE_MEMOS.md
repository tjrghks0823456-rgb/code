...
### 12. 문답/업로드 진행 버튼 비활성 UX 개선
- 무엇을 수정했나: `survey/page.tsx`에서 다음/결과 버튼을 무조건 비활성화하지 않고, 누르면 현재 단계에서 남은 문항 수를 안내하도록 바꿨다. 현재 단계의 선택 완료 개수와 진행 바도 추가했다.
- 왜 수정했나: 기존에는 4개 문항을 모두 선택해야 `다음` 버튼이 활성화되어, 사용자가 일부만 선택했을 때 버튼이 고장난 것처럼 보였다.
- 업로드 반영: `upload/page.tsx`에서 개인정보 동의 전 `시청 기록 선택하기`, 파일 선택 전 `분석 준비 화면으로` 버튼을 누를 수 있게 유지하고, 조건이 부족하면 안내 메시지를 띄우도록 바꿨다.
- 파일 선택 보강: 업로드 박스 전체를 클릭하면 ZIP 파일 선택창이 열리도록 했고, 내부의 `Takeout 폴더 선택`/`ZIP 파일 선택` 버튼은 각각 명확히 동작하도록 이벤트 전파를 정리했다. 같은 파일을 다시 골라도 change 이벤트가 다시 발생하도록 input 값을 클릭 시 초기화했다.
- 확인 방법: 프론트 `node.exe .\node_modules\typescript\bin\tsc --noEmit`를 통과했다. 브라우저에서 `http://127.0.0.1:3000/survey`, `http://127.0.0.1:3000/upload`를 열어 버튼이 계속 클릭 가능한 상태로 보이고, 조건 부족 시 안내가 표시되는 것을 확인했다.

### 13. 원클릭 런처 브라우저 오픈 대기 보강
- 무엇을 수정했나: `open_unbelievable.ps1`에 `Wait-PortOpen` 함수를 추가해 백엔드 8000번과 프론트 3000번 포트가 실제로 열릴 때까지 기다린 뒤 브라우저를 열도록 변경했다.
- 왜 수정했나: 기존 런처는 서버 실행 후 고정 8초만 기다리고 브라우저를 열어, Next dev server 초기 컴파일이 느린 경우 `localhost 연결을 거부했습니다`가 먼저 뜰 수 있었다.
- 확인 방법: PowerShell parser로 `open_unbelievable.ps1` 문법 오류가 없는 것을 확인했고, 현재 `http://127.0.0.1:3000/`와 `http://127.0.0.1:8000/docs`가 정상 응답하는 것을 확인했다.

### 14. 대시보드 검색 맵 중복 제거 및 Gemini 분류 방향 메모
- 무엇을 수정했나: `dashboard/page.tsx`에서 하단 레이더 차트 옆에 다시 렌더링되던 `검색어 관심사 맵`을 제거했다. 검색 맵은 상단 `관심사 비교 리포트`의 큰 `검색 기반 맵`에서만 보여준다.
- 왜 수정했나: 같은 검색 기반 관심사 맵이 위에는 크게, 아래에는 작게 한 번 더 나와 화면이 반복적으로 보이고 정보 위계가 흐려졌다. 하단에는 실제 검색어 근거 카드만 남겨 증거 목록 역할로 정리했다.
- Gemini API 방향: Gemini는 인명, 곡명, 채널명, 짧은 영문 제목처럼 규칙 기반으로 분류하기 어려운 `기타/미분류`를 줄이는 데 도움이 된다. 다만 모든 원문을 매번 보내기보다는 규칙 분류 실패 항목과 상위 미분류 샘플만 배치로 보내고, 결과를 캐싱해 비용과 개인정보 노출을 줄이는 방식이 적절하다.
- 한계: Gemini를 붙여도 Google Takeout에 없는 숏츠 URL이나 누락된 데이터까지 복구할 수는 없다. `shorts` 감지 문제는 URL/메타데이터 수집 방식 문제로 별도 처리해야 한다.
