...
class DashboardService:
    ...
    def normalize_exception_codes(self, raw_codes: Any) -> List[str]:
        if not raw_codes:
            return []
    ...
    def build_score_warnings(self, exception_codes: List[str]) -> List[Dict[str, str]]:
        warnings = []
        seen_codes = set()
    ...
    def is_known_value(self, value: Any) -> bool:
        return self.clean_text(value).lower() not in UNKNOWN_SOURCE_VALUES

    def top_level_category(self, category: Any) -> str:
    ...
    def build_distribution_shares(self, counter: Counter, top_limit: int = 6) -> List[Dict[str, Any]]:
    ...
    def build_dashboard_insights(
        ...
        for event in analysis_events:
            title = self.clean_text(event.get("text_base"))
            ...
            channel = self.clean_text(event.get("channel_url") or event.get("channel_name") or event.get("author_id") or event.get("source_surface"))
            if is_standard_video_event(event) and self.is_known_value(channel):
        ...
        for result in nlp_results:
            ...
            for category in categories:
                topic = self.top_level_category(category)
                if topic:
                    topic_counter[topic] += 1

        topic_shares = self.build_distribution_shares(topic_counter)
        channel_shares = self.build_distribution_shares(channel_counter, top_limit=5)
        search_keywords = [
            {
                "keyword": keyword,
                "count": count,
    ...
    def get_dashboard_summary(self, run_id: str, user_id: str) -> Dict[str, Any]:
        # 1. Fetch score run
        ...
        exception_codes = self.normalize_exception_codes(run.get("exception_codes", []))
        score_warnings = self.build_score_warnings(exception_codes)
        ...
        # Calculate Actual DSAO based on actual watch data
        file_id = run["file_id"]
        raw_files = self.repository.fetch_raw_file(file_id)
        raw_file = raw_files[0] if raw_files else {}
        events = self.repository.fetch_norm_events(file_id)

        # Calculate Debug Counts / Data Quality Summary
        total_watch_events = 0
        watch_url_events = 0
        shorts_url_events = 0
        shorts_title_hint_events = 0
        shorts_inferred_events = 0
        general_watch_inferred_events = 0
        unknown_classification_events = 0
        category_missing_events = 0
        has_takeout_estimation = False

        timestamp_parse_failed_count = 0
        duration_method_metadata_count = 0
        duration_method_timeline_gap_count = 0
        ...
        # Check if duration is estimated
        has_estimated_duration = any(e.get("is_duration_estimated") for e in events)

        for e in events:
            act_type = (e.get("action_type") or "").lower()
            src_type = (e.get("source_type") or "").lower()
            is_watch = act_type == "view" or src_type == "watch_history"
            if not is_watch:
                continue

            total_watch_events += 1

            url = (e.get("title_url") or e.get("titleUrl") or e.get("url") or e.get("URL") or "").lower()
            text = (e.get("text_base") or e.get("title_text") or e.get("title") or "").lower()
            ...
            if "/shorts/" in url or "youtube.com/shorts" in url:
                ...
                if any(hint in text for hint in hints):
                    shorts_title_hint_events += 1

            fmt = e.get("content_format")
            is_shorts_cand = e.get("is_shorts_candidate")
            is_general_cand = e.get("is_general_watch_candidate")

            if fmt == "shorts" or is_shorts_cand is True:
                shorts_inferred_events += 1
            elif fmt == "standard_video" or is_general_cand is True:
                general_watch_inferred_events += 1
            else:
                unknown_classification_events += 1
            ...
            raw_item = e.get("raw_item") or {}
            ts_failed = e.get("timestamp_parse_failed")
            if ts_failed is None:
                ts_failed = raw_item.get("timestamp_parse_failed")
            if ts_failed is None:
                ts_failed = (e.get("event_time") is None)
            if ts_failed:
                timestamp_parse_failed_count += 1

            # Duration estimation method check
            method = e.get("duration_estimation_method")
            if not method:
                method = raw_item.get("duration_estimation_method")
            if not method:
                ds = e.get("duration_source") or raw_item.get("duration_source")
                if ds == "youtube_api" or ds == "api" or ds == "timeline_capped_api":
                    method = "metadata"
                elif ds == "timeline_gap":
                    method = "timeline_gap"
                elif ds == "idle_capped":
                    method = "idle_capped"
                elif ds == "default_estimate":
                    method = "default_estimate"
                else:
                    method = "unknown"

            if method == "metadata":
                duration_method_metadata_count += 1
            elif method == "timeline_gap":
                duration_method_timeline_gap_count += 1
            elif method == "idle_capped":
                duration_method_idle_capped_count += 1
            elif method == "default_estimate":
                duration_method_default_estimate_count += 1
            else:
                duration_method_unknown_count += 1
        ...
        data_quality_summary = {
            ...
            "shorts_title_hint_events": shorts_title_hint_events,
            "shorts_inferred_events": shorts_inferred_events,
            "general_watch_inferred_events": general_watch_inferred_events,
        ...
        if category_missing_ratio >= 0.8:
            ...
            if "TDS" in score_details:
                score_details["TDS"]["confidence"] = 0.0
            
            if "P17_CATEGORY_MISSING_EXCESSIVE" not in exception_codes:
                exception_codes.append("P17_CATEGORY_MISSING_EXCESSIVE")

        analysis_events, excluded_ad_events = split_analysis_events(events)
        nlp_results = self.repository.fetch_nlp_results_for_file(file_id)
        view_events = [
        ...
        insights = self.build_dashboard_insights(events, nlp_results, axis_scores, score_warnings)
        ...
        # Calculate Explanations for key scores
        ...
        return {
            ...
            "analysis_debug": data_quality_summary,
            "user": {
